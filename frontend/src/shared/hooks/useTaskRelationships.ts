...
export const useTaskRelationships = (tasks: Task[], setTasks: React.Dispatch<React.SetStateAction<Task[]>>) => {
  ...
  const getDependents = useCallback((taskId: string, currentTasks: Task[] | null = null): Task[] => {
    const tasksToUse = currentTasks || tasks;
  ...
  const calculateDependentStatus = useCallback((dependencies: Task[]): TaskStatus | null => {
  ...
  const updateDependentStatus = useCallback((taskId: string, currentTasks: Task[] | null = null) => {
    const tasksToUse = currentTasks || tasks;
  ...
  const getBlockers = useCallback((task: Task): TaskBlocker[] => {
    ...
    // Find tasks that this task depends on and are blocked or in progress
    if (task.depends && task.depends.length > 0) {
      task.depends.forEach(depId => {
        const dependency = tasks.find(t => t.id === depId);
        if (dependency && (dependency.status === 'blocked' || dependency.status === 'in-progress')) {
          blockers.push({
            by: dependency.id,
            owner: dependency.owner,
            type: 'dependency',
            since: dependency.lastUpdated || new Date().toISOString()
          });
        }
      });
    }
