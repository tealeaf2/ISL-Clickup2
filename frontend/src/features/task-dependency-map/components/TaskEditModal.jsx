 ...
 * 
 * Modal form for editing existing tasks. Displays all task properties as editable
 * fields and allows saving changes or deleting the task. This is a presentational
 * component - all state is managed by the parent container.
 * 
 * @param {Object} props - Component props
 * @param {boolean} props.isOpen - Whether the modal is visible
 * @param {Object} props.task - The task being edited (for reference)
 * @param {Object} props.draft - Current draft state with edited values
...
const TaskEditModal = ({ 
  ...
  return (
    <Modal onClose={onClose} title={draft?.name || `Edit Task ${task.id}`}>
      <div className="space-y-2">
        <div className="text-xs text-gray-500 mb-2">ID: {draft?.id}</div>
        <InputRow
          label="Name"
          value={draft?.name || ''}
