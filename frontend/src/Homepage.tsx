...
import { computeRisk } from "./riskCalculator"; // Import the risk calculator
...
type Task = {
  ...
  subtasks?: Task[];
  duration?: number;
  dependencies?: { total: number; blocked: number };
...
const TASKS: Task[] = [
  {
    ...
    duration: 8,
    dependencies: { total: 2, blocked: 1 },
    last_update_days: 3,
    subtasks: [
      {
        id: "1.1",
        ...
        duration: 3,
        dependencies: { total: 0, blocked: 0 },
        last_update_days: 10,
      },
      {
        id: "1.2",
        ...
        duration: 4,
        dependencies: { total: 1, blocked: 0 },
        last_update_days: 2,
      },
    ],
  },
  {
    ...
    duration: 5,
    dependencies: { total: 3, blocked: 2 },
    last_update_days: 8,
    subtasks: [
      {
        id: "2.1",
  ...
  },
  {
    ...
    duration: 6,
    dependencies: { total: 4, blocked: 0 },
    last_update_days: 1,
    subtasks: [
      {
        id: "4.1",
      ...
      },
      {
        ...
        priority: "Medium",
        duration: 3,
        dependencies: { total: 1, blocked: 1 },
        last_update_days: 9,
      },
    ],
  },
...
// --- CONSTANTS (UPDATED FOR RISK COLUMN) ---
const COL_WIDTHS = {
  assignee: 160,
  dueDate: 120,
  priority: 100,
  status: 120,
  risk: 100, // Added width for the new Risk column
};
const nameCalc = `calc(100% - ${160 + 120 + 100 + 120 + 100}px)`; // Adjusted calculation
...
const getRiskAppearance = (score: number) => {
  if (score > 60) return { bg: "#fff1f2", text: "#b91c1c" }; // High
  if (score > 30) return { bg: "#fffbeb", text: "#b45309" }; // Medium
  return { bg: "#f0fdf4", text: "#059669" }; // Low
};
...
// NEW: A recursive function to add risk scores to tasks and their subtasks
function addRiskScores(task: Task): Task & { risk: number } {
  return {
    ...task,
    risk: computeRisk(task),
    subtasks: task.subtasks?.map(addRiskScores),
  };
}
...
export default function TaskPage() {
  ...
  const grouped = useMemo(() => {
    const tasksWithRisk = TASKS.map(addRiskScores); // Add risk scores to tasks

    const map = new Map<string, (Task & { risk: number })[]>();
    for (const t of tasksWithRisk) {
    ...
    const groupedSorted = sortedKeys.map((k) => {
      const arr = map.get(k)!.slice().map((t) => {
        const clone: Task & { risk: number } = { ...t };
        if (clone.subtasks) {
          clone.subtasks = sortTasksByDueDateAndName(clone.subtasks).map((st) => {
            const deep = { ...(st as Task & { risk: number }) };
      ...
      return [k, arr] as [string, (Task & { risk: number })[]];
    });
    return groupedSorted;
  }, []);

  const renderTaskRows = (
    task: Task & { risk: number },
    depth = 0
  ): React.ReactNode => {
    ...
    const riskAppearance = getRiskAppearance(task.risk); // NEW
    ...
    return (
      <React.Fragment key={task.id}>
        ...
        >
          ...
          <td style={{ padding: "10px 12px", width: COL_WIDTHS.risk, verticalAlign: "middle" }}>
             <div
              style={{
                fontSize: 13,
                padding: "6px 10px",
                ...
                gap: 6,
                fontWeight: 600,
                background: riskAppearance.bg,
                color: riskAppearance.text,
              }}
            >
              {task.risk}
            </div>
        ...
        {hasSub && isOpen ? (
          task.subtasks!.map((st) => <React.Fragment key={st.id}>{renderTaskRows(st as Task & { risk: number }, depth + 1)}</React.Fragment>)
        ) : null}
      </React.Fragment>
    );
  };
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
                <thead>
                  <tr style={{ textAlign: "left", fontSize: 13, color: "#6b7280", borderTop: "1px solid #f3f4f6" }}>
                    <th style={{ padding: "10px 12px", width: nameCalc }}>Name</th>
                    ...
                    <th style={{ padding: "10px 12px", width: COL_WIDTHS.risk }}>Risk</th>
